...
preference* exploration_choose_according_to_policy(agent* thisAgent, slot* s, preference* candidates)
{
    ...
    if (my_rl_enabled)
    {
        ...
        {
            rl_perform_update(thisAgent, return_val, candidates, return_val->rl_contribution, s->id); ///<bazald
        }
        else if (my_learning_policy == rl_param_container::q)
        {
            rl_perform_update(thisAgent, top_candidate, candidates, top_rl, s->id); ///<bazald
