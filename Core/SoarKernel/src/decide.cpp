...
//                       << " ^" << w->attr->sc->name
//                       << " " << int(w->value.symbol_type) << std::endl;
            if(w->attr->symbol_type == STR_CONSTANT_SYMBOL_TYPE &&
               w->value->symbol_type == STR_CONSTANT_SYMBOL_TYPE &&
...
void update_impasse_items(agent* thisAgent, Symbol* id, preference* items)
{
    ...
    for (int it = regular; it <= numeric; it++)
    {
        ...
        for (cand = items; cand != NIL; cand = cand->next_candidate)
        {
            ...
            if(cand->rl_intolerable_variance && cands_over_threshold.find(cand) == cands_over_threshold.end()) {
...
void remove_existing_context_and_descendents(agent* thisAgent, Symbol* goal)
{
    ...
    if ((goal != thisAgent->top_goal) && rl_enabled(thisAgent))
    {
        ...
        rl_perform_update(thisAgent, 0, 0, true, goal, false); ///< bazald  // this update only sees reward - there is no next state
        rl_clear_refs(goal);
    }
