...
rl_param_container::rl_param_container(agent* new_agent): soar_module::param_container(new_agent)
{
    ...
    /// bazald's influence discount-rate
    ...
    /// bazald's eligibility-trace-type
    et_type = new soar_module::constant_param<trace_choices>( "eligilibity-trace-type", trace_eligibility, new soar_module::f_predicate<trace_choices>() );
    et_type->add_mapping( trace_eligibility, "eligibility" );
    et_type->add_mapping( trace_tsdt, "tsdt" );
    add( et_type );

    /// bazald's tsdt-cutoff
    ...
    /// bazald's rl-impasse
    rl_impasse = new soar_module::boolean_param( "rl-impasse", off, new soar_module::f_predicate<boolean>() ); ///< bazald
    add( rl_impasse );

    /// bazald's fc-credit
    credit_assignment = new rl_credit_assignment_param( "credit-assignment", credit_even, new soar_module::f_predicate<credit_assignment_choices>(), new_agent );
    ...
    /// bazald's fc-credit
    credit_modification = new rl_credit_modification_param( "credit-modification", credit_mod_none, new soar_module::f_predicate<credit_modification_choices>(), new_agent );
    ...
    /// bazald's variance-bellman
    variance_bellman = new soar_module::boolean_param( "variance-bellman", on, new soar_module::f_predicate<boolean>() ); ///< bazald
    ...
    /// bazald's refine-require-episodes
    ...
    /// bazald's refine-cycles-between-episodes
    ...
    /// bazald's refine-reinhibit
    refine_reinhibit = new soar_module::boolean_param( "refine-reinhibit", on, new soar_module::f_predicate<boolean>() );
        ...
        if (s)
        {
            for (w = s->wmes; w; w = w->next)
            {
                if (w->value->symbol_type == IDENTIFIER_SYMBOL_TYPE)
                {
                    ...
                    t = find_slot( w->value, thisAgent->rl_sym_terminal );
                    if(t) {
                        data->terminal = true;
...
void rl_perform_update(agent* thisAgent, preference *selected, preference *candidates, bool op_rl, Symbol* goal, bool update_efr) ///< bazald
{
    const double op_value = selected ? selected->numeric_value : 0.0; ///< bazald
    bool using_gaps = (thisAgent->rl_params->temporal_extension->get_value() == on);
    ...
    if (!using_gaps || op_rl)
    {
        ...
        if (!data->prev_op_rl_rules->empty())
        {
            ...
            if(thisAgent->rl_params->et_type->get_value() == rl_param_container::trace_tsdt) {
                data->eligibility_traces->clear();

                if(data->terminal)
                    data->tsdt_trace->insert(thisAgent, new TSDT_Terminal(*data->prev_op_rl_rules, data->reward, data->terminal_reward));
                else if(update_efr) {
                    if(thisAgent->rl_params->learning_policy->get_value() == rl_param_container::q)
                        data->tsdt_trace->insert(thisAgent, new TSDT_Q(*data->prev_op_rl_rules, data->reward, candidates));
                    else
                        data->tsdt_trace->insert(thisAgent, new TSDT_Sarsa(*data->prev_op_rl_rules, data->reward, selected));
                }
                else
                    data->tsdt_trace->insert(thisAgent, new TSDT_Interrupted(*data->prev_op_rl_rules, data->reward));

                data->tsdt_trace->update(thisAgent);
            }
            else {
...
//                     double trace_increment = (1.0 / static_cast<double>(data->prev_op_rl_rules->size()));
                ...
                // For each prod with a trace, perform update
                {
                    ...
                    for (iter = data->eligibility_traces->begin(); iter != data->eligibility_traces->end(); iter++)
                    {
                        ...
                        if(thisAgent->rl_params->decay_mode->get_value() == rl_param_container::adaptive_decay) {///< bazald, See Adaptive Step-Size for Online Temporal Difference Learning (William Dabney and Andrew G. Barto)
                            const float denom = fabs(delta_ecr + delta_efr);
                            if(denom > 0.0) {
                                const double new_adjusted_alpha = 1.0 / denom;
                                if(new_adjusted_alpha < alpha) {
                                    thisAgent->rl_params->learning_rate->set_value(new_adjusted_alpha);
                                    alpha = new_adjusted_alpha;
                                }
                            }
                        }

                        const double rdr = thisAgent->rl_params->refine_decay_rate->get_value(); ///< bazald
                        if(rdr == 1.0)
                            prod->rl_update_amount += fabs(data->reward + op_value - old_combined); ///< bazald
                        else {
                            prod->rl_update_num *= rdr;
                            prod->rl_update_num += fabs(data->reward + op_value - old_combined);
    ...
    //                     prod->rl_update_amount += fabs(delta_ecr + delta_efr); ///< bazald
                        ...
                        if(prod->init_updated_last == thisAgent->init_count) { ///< bazald
                            ...
                            if(!thisAgent->rl_params->refine_reinhibit->get_value())
                                prod->total_updated_last = thisAgent->decision_phases_count; ///< bazald
                            prod->init_updated_last = thisAgent->init_count; ///< bazald
                            ++prod->init_updated_count; ///< bazald
      //                       std::cerr << prod->name->sc.name << " updated for " << prod->init_updated_count << " episodes as of " << thisAgent->init_count << std::endl;
                        }
                        if(thisAgent->rl_params->refine_reinhibit->get_value())
                            prod->total_updated_last = thisAgent->decision_phases_count; ///< bazald
                        ...
                        if(thisAgent->rl_params->variance_bellman->get_value()) { /// bazald: Variance calculation
...
void TSDT::update(agent * const &thisAgent) {
...
void TSDT::update_efr(agent * const &thisAgent) {
  const double alpha = thisAgent->rl_params->learning_rate->get_value();
  const double discount = thisAgent->rl_params->discount_rate->get_value();
...
void TSDT_Interrupted::update_efr(agent * const &thisAgent) {
}
...
void TSDT_Trace::insert(agent * const &thisAgent,
                        TSDT * given_TSDT)
{
...
void TSDT_Trace::update(agent * const &thisAgent) {
  trim(thisAgent);
  ...
  for(std::set<production *>::iterator pdl = productions.begin(), pdlend = productions.end(); pdl != pdlend; ++pdl) {
    ...
    symbol_remove_ref(thisAgent, rhs_value_to_symbol((*pdl)->action_list->referent));
    (*pdl)->action_list->referent = symbol_to_rhs_value(make_float_constant(thisAgent, (*pdl)->rl_ecr + (*pdl)->rl_efr));

    for(instantiation *inst = (*pdl)->instantiations; inst; inst = inst->next) {
      for(preference *pref = inst->preferences_generated; pref; pref = pref->inst_next) {
        symbol_remove_ref(thisAgent, pref->referent);
        pref->referent = make_float_constant(thisAgent, (*pdl)->rl_ecr + (*pdl)->rl_efr);
      }
    }
  }
}

void TSDT_Trace::trim(agent * const &thisAgent) {
  while(length > thisAgent->rl_params->tsdt_cutoff->get_value()) {
    delete trace.back();
    trace.pop_back();
    --length;
  }
