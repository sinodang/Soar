...
//        }
    ...
    if (pref->type == NUMERIC_INDIFFERENT_PREFERENCE_TYPE)
    {
        ...
        for (s = pref->id->id->slots; s; s = s->next)
        {
            ...
            {
                if (p->id == pref->id && p->attr->is_string() &&
...
//                        insert_at_head_of_dll(pref->next_numeric, pref, next_numeric, prev_numeric);
                        ...
                        insert_at_head_of_dll(p->next_numeric, pref, next_numeric, prev_numeric);
                        pref->prev_numeric = p;
                        dprint(DT_EBC_RL, "Found(+) ");
                    }
                    else
                    {
                        p->next_numeric = pref;
                        pref->prev_numeric = p;
                        dprint(DT_EBC_RL, "Found(0) ");
                    }
                    preference_add_ref(pref);
                    foundPref = true;
                    dprint_noprefix(DT_EBC_RL, "acceptable pref %p (i%u %y) to add for pref %p (i%u %y)\n", p, p->inst->i_id, p->inst->prod->name, pref, pref->inst->i_id, pref->inst->prod->name);
                    dprint(DT_EBC_RL, "...next_numeric list: ");
                    for (preference *p2 = p->next_numeric; p2; p2 = p2->next_numeric)
                    {
                        dprint_noprefix(DT_EBC_RL, "%p ", p2);
                    }
                }
            }
        }
        if (!foundPref) dprint(DT_EBC_RL, "Did not find acceptable pref to add for pref %p (%u)\n", pref, pref->inst->i_id);
