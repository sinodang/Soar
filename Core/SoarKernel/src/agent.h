...
//typedef std::unordered_map<production *, rl_qconf_data, std::less<production *>, soar_module::soar_memory_pool_allocator<std::pair<production* const, rl_qconf_data> > > rl_qconf_map;
typedef std::unordered_map<production *, rl_qconf_data> rl_qconf_map;
#else
typedef std::unordered_map<production *, rl_qconf_data> rl_qconf_map;
#endif
#endif
...
typedef struct EXPORT agent_struct
{
    ...
    #ifdef CHUNKING_WITH_CONFIDENCE
    double              chunk_prob; /* probability associated with the current chunk */
    rl_qconf_map        *rl_qconf; /* mapping from productions to Q(s,a) data */
    #endif
